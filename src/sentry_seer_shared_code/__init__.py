...
Pydantic Version Support:
    ...
    - Use `sentry_seer_shared_code.v1.*` for Pydantic v1 (pydantic<2)
    - Use `sentry_seer_shared_code.v2.*` for Pydantic v2 (pydantic>=2) - reserved for future use

    Currently, all models are in v1 which works with both Pydantic versions.

Examples:
    # In Sentry (Pydantic v1)
    from sentry_seer_shared_code.v1 import SeerCodeReviewConfig
